...
# Convenience: pick a single valid (park, site, param) triple from the fixture
pick_one_case <- function(wd) {
  cases <- sample_n_valid_combos(wd, n_cases = 1, seed = getOption("ncrnwater.test.seed", 42))
  cases[[1]]
}
...
test_that("site_info returns expected fields; site_data returns measurable columns", {
  wd    <- getWD()
  combo <- pick_one_case(wd); park <- combo$park; site <- combo$site; param <- combo$param
